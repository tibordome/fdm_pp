...
def createCatMajorCOMGx():
    ...
    if rank == 0:
        ...
        rdelta = list(np.loadtxt('{0}/r_delta_fdm_dm_{1}.txt'.format(config.CAT_DEST, config.SNAP)))
        sh_com = np.loadtxt('{0}/sh_coms_fdm_{1}.txt'.format(config.CAT_DEST, config.SNAP))
    else:
        sh_cat = None
        rdelta = None
        sh_com = None
    ...
    if rank == 0:
        ...
        np.savetxt('{0}/eps_9_{1}_gx_{2}.txt'.format(config.CAT_DEST, config.DM_TYPE, config.SNAP), np.array([eps_9]), fmt='%1.7e')
    
    # Morphology
    print_status(rank, start_time, "Calculating local-shape morphologies. The average number of ptcs in the gxs is {0}".format(np.average(np.array(list(map(lambda x: len([x for x in gx_cat if x != []][x]), range(len([x for x in gx_cat if x != []]))))))))
    ...
    if rank == 0:
        ...
        # Writing
        with open('{0}/a_com_cat_local_fdm_gx.txt'.format(config.CAT_DEST), 'w') as filehandle:
            json.dump(a_com_cat_gx, filehandle)
       
        # Storing np.arrays
        np.savetxt('{0}/d_local_fdm_gx.txt'.format(config.CAT_DEST), d, fmt='%1.7e')
        np.savetxt('{0}/q_local_fdm_gx.txt'.format(config.CAT_DEST), q, fmt='%1.7e')
        np.savetxt('{0}/s_local_fdm_gx.txt'.format(config.CAT_DEST), s, fmt='%1.7e')
        np.savetxt('{0}/minor_local_fdm_gx.txt'.format(config.CAT_DEST), minor_new_re, fmt='%1.7e')
        np.savetxt('{0}/inter_local_fdm_gx.txt'.format(config.CAT_DEST), inter_new_re, fmt='%1.7e')
        np.savetxt('{0}/major_local_fdm_gx.txt'.format(config.CAT_DEST), major_new_re, fmt='%1.7e')