...
def createCatMajorCOMDM(start_time):          
    ...
    if rank == 0:
        ...
        while sh_idx < len(cat):
            ...
            sh_idx += 1
    cat = comm.bcast(cat, root = 0)
    print_status(rank, start_time, "The number of SHs after discarding low-resolution ones, before morphology calculation is {0}".format(np.array([0 for x in cat if x != []]).shape[0]))
    ...
    if rank == 0:
        ...
        np.savetxt('{0}/eps_9_fdm_dm_{1}.txt'.format(config.CAT_DEST, config.SNAP), np.array([eps_9]), fmt='%1.7e')
        
    # Local Morphology
    print_status(rank, start_time, "Calculating local morphologies. The average number of ptcs in the SHs is {0}".format(np.average(np.array(list(map(lambda x: len(cat[x]), range(len(cat))))))))
    ...
    print_status(rank, start_time, "Calculating overall morphologies. The average number of ptcs in the SHs is {0}".format(np.average(np.array(list(map(lambda x: len([x for x in cat if x != []][x]), range(len([x for x in cat if x != []]))))))))
    d, q, s, minor, inter, major, shs_com, sh_m = getMorphologies(dm_xyz, cat, dm_masses, rdelta, "dm", "overall", start_time)
    print_status(rank, start_time, "Gotten morphologies")
    
    if rank == 0:
        if d != []:
            d = np.reshape(np.array(d), (np.array(d).shape[0], np.array(d).shape[1])) # Has shape (number_of_shs, 1), each d_discr a float
            q = np.reshape(np.array(q), (np.array(q).shape[0], np.array(q).shape[1])) # Has shape (number_of_shs, 1), each d_discr a float
            s = np.reshape(np.array(s), (np.array(s).shape[0], np.array(s).shape[1])) # Has shape (number_of_shs, 1), each d_discr a float
            minor = np.array(minor) # Has shape (number_of_shs, 1), each of (3,) shape
            inter = np.array(inter) # Has shape (number_of_shs, 1), each of (3,) shape
            major = np.array(major) # Has shape (number_of_shs, 1), each of (3,) shape
            major_new = np.zeros((major.shape[0], major.shape[1], 3)) # Has shape (number_of_shs, 1, 3) finally
        ...
        # Create catalogue storing major axes at "maximal" r_ell (just so that all particles are inside) and sh_com
        a_com_cat = [[] for i in range(d.shape[0])] # For each sh, 6 floats, first 3 give orientation of major axis at maximal r_ell, last 3 give sh_com's x, y, z
        ...
        # Writing
        with open('{0}/a_com_cat_overall_fdm_dm_{1}.txt'.format(config.CAT_DEST, config.SNAP), 'w') as filehandle:
            json.dump(a_com_cat, filehandle)
            
        with open('{0}/sh_cat_overall_fdm_{1}.txt'.format(config.CAT_DEST, config.SNAP), 'w') as filehandle:
            json.dump(cat, filehandle)
            
        # Storing np.arrays
        np.savetxt('{0}/d_overall_fdm_dm_{1}.txt'.format(config.CAT_DEST, config.SNAP), d, fmt='%1.7e')
        np.savetxt('{0}/q_overall_fdm_dm_{1}.txt'.format(config.CAT_DEST, config.SNAP), q, fmt='%1.7e')
        np.savetxt('{0}/s_overall_fdm_dm_{1}.txt'.format(config.CAT_DEST, config.SNAP), s, fmt='%1.7e')
        np.savetxt('{0}/minor_overall_fdm_dm_{1}.txt'.format(config.CAT_DEST, config.SNAP), minor_new_re, fmt='%1.7e')
        np.savetxt('{0}/inter_overall_fdm_dm_{1}.txt'.format(config.CAT_DEST, config.SNAP), inter_new_re, fmt='%1.7e')
        np.savetxt('{0}/major_overall_fdm_dm_{1}.txt'.format(config.CAT_DEST, config.SNAP), major_new_re, fmt='%1.7e')