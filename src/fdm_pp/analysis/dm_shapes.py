...
def dm_shapes(start_time):
    ...
    if rank == 0:
        ...
        # Reading
        a_com_cat_fdm, h_cat_fdm, d_fdm, q_fdm, s_fdm, major_fdm_full, r200_fdm, fof_masses_fdm = readDataFDM(get_skeleton = False)
        eps_9_fdm = np.loadtxt('{0}/eps_9_fdm_dm_{1}.txt'.format(config.CAT_DEST, config.SNAP))
        print_status(rank, start_time, "The number of FDM halos considered is {0}".format(len(a_com_cat_fdm)))
        ...
        if config.HALO_REGION == "Inner":
            ...
            plt.ylim(0.0, 1.0)
            plt.savefig("{0}/dm/Haloq_{1}.pdf".format(config.SHAPE_DEST, config.SNAP), bbox_inches="tight")
            ...
            plt.legend(); plt.legend(fontsize="small")
            plt.ylim(0.0, 1.0)
            plt.savefig("{0}/dm/Halos_{1}.pdf".format(config.SHAPE_DEST, config.SNAP), bbox_inches="tight")
            
            # T
            plt.figure()
            if q_fdm.ndim == 2:
                T_fdm = np.zeros((q_fdm.shape[0], q_fdm.shape[1]))
                for halo in range(q_fdm.shape[0]):
                    T_fdm[halo] = (1-q_fdm[halo]**2)/(1-s_fdm[halo]**2) # Triaxiality
            else:
                T_fdm = np.empty(0)
            ...
            plt.legend(); plt.legend(fontsize="small")
            plt.ylim(0.0, 1.0)
            plt.savefig("{0}/dm/HaloT_{1}.pdf".format(config.SHAPE_DEST, config.SNAP), bbox_inches="tight")
            ...
            plt.legend(); plt.legend(fontsize="small")
            plt.ylim(0.0, 1.0)
            plt.savefig("{0}/dm/HaloqMs_{1}.pdf".format(config.SHAPE_DEST, config.SNAP), bbox_inches="tight")
            ...
            plt.legend(); plt.legend(fontsize="small")
            plt.ylim(0.0, 1.0)
            plt.savefig("{0}/dm/HalosMs_{1}.pdf".format(config.SHAPE_DEST, config.SNAP), bbox_inches="tight")
            ...
            plt.legend(); plt.legend(fontsize="small")
            plt.ylim(0.0, 1.0)
            plt.savefig("{0}/dm/HalosTMs_{1}.pdf".format(config.SHAPE_DEST, config.SNAP), bbox_inches="tight")
        
        # T counting
        plt.figure()
        t_fdm[t_fdm == 0.] = np.nan
        n, bins, patches = plt.hist(x=t_fdm, bins = np.linspace(0, 1, config.HIST_NB_BINS), alpha=0.7, density=True, label = "FDM Halos")
        ...
        plt.legend(); plt.legend(fontsize="small")
        plt.xlim(0.0, 1.0)
        plt.savefig("{0}/dm/{1}HaloTCount_{2}.pdf".format(config.SHAPE_DEST, config.HALO_REGION, config.SNAP), bbox_inches="tight")
                
        t_fdm = t_fdm[np.logical_not(np.isnan(t_fdm))]
        print_status(rank, start_time, "{0}. In degrees: The average T value for FDM Halos is {1} and the standard deviation (assuming T is Gaussian distributed) is {2}".format(config.HALO_REGION, round(np.average(t_fdm),2), round(np.std(t_fdm),2)))
    ...
    if config.HALO_REGION == "Full":
        ...
        if rank == 0:
            ...
            n, bins, patches = plt.hist(x=abs(eps_fdm), bins = np.linspace(0, 1, config.HIST_NB_BINS), alpha=0.7, density=True, label = "FDM Halos")
            plt.xlabel(r"$\epsilon$")
            plt.ylabel('Normalized Bin Count')
            plt.grid(axis='y', alpha=0.75)
            plt.legend(); plt.legend(fontsize="small")
            plt.xlim(0.0, 1.0)
            plt.savefig("{0}/dm/{1}HaloEpsCount_{2}.pdf".format(config.SHAPE_DEST, config.HALO_REGION, config.SNAP), bbox_inches="tight")