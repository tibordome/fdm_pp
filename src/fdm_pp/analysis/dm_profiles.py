...
def dm_profiles(start_time):
    ...
    if rank == 0:
        ...
        print_status(rank, start_time, "The number of FDM shs considered is {0}".format(d_fdm.shape))
        
        # Assembly
        sh_masses_fdm, sh_com_arr_fdm, idx_fdm, major_fdm, t_fdm = assembleDataFDM(a_com_cat_fdm, h_cat_fdm, d_fdm, q_fdm, s_fdm, major_fdm_full, fof_masses_fdm)
        ...
        if config.HALO_REGION == "Inner":
            ...
            if d_fdm.ndim == 2:
                ...
                eps_9_fdm = np.nan # We are dealing with empty arrays, i.e. no shs considered
            ...
            for sh in range(rho_profs.shape[0]):
                plt.figure()
                plt.semilogx(R, rho_profs[sh], label="SH {0}".format(sh))
                plt.xlabel(r"$r/R_{200}$")
                plt.ylabel(r"$\rho$ [$10^{10} M_{{\odot}}$ / Mpc/$h^2$]")
                if d_fdm.ndim == 2:
                    eps_9_fdm = np.average(eps_9_fdm/d_fdm[:,-int(config.D_LOGEND/((config.D_LOGEND-config.D_LOGSTART)/config.D_BINS))-1])
